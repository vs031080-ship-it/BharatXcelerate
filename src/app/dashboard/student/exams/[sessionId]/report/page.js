        ...
        <div className={styles.page}>
            <div className={styles.header}>
                <div className={styles.title}>
                    Detailed Report: {session.testPaper?.badgeLabel || session.testPaper?.category?.name || 'Exam'}
                </div>
                <button className={styles.backBtn} onClick={() => router.push('/dashboard/student/exams')}>
                    <ChevronLeft size={16} /> Back to Exams
                </button>
            ...
            <div className={`${styles.scoreBanner} ${session.passed ? styles.scoreBannerPass : styles.scoreBannerFail}`}>
                ...
                {session.passed && session.testPaper?.badgeLabel && (
                    <div className={styles.badgeEarned}>
                        <Award size={20} /> Earned: {session.testPaper.badgeLabel}
                    </div>
                )}
            </div>
            ...
            <p style={{ marginBottom: '1.5rem', color: '#64748b' }}>
                You attempted {attemptedCount} out of {totalQuestions} questions. Unattempted questions are hidden from this report.
            </p>
                            ...
                            {q.options.map((opt, optIdx) => {
                                ...
                                return (
                                    <div key={optIdx} className={optionClass}>
                                        ...
                                        {isSelected && q.isCorrect && (
                                            <div className={`${styles.optionBadge} ${styles.badgeCorrect}`}>
                                                <CheckCircle2 size={14} /> You Selected
                                            </div>
                                        )}
                                    </div>
                                );
