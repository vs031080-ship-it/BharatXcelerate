    ...
    return (
        <div className={styles.container} style={{ maxWidth: '1200px' }}>
            ...
            {activeTab === 'skills' && (
                <div style={{ paddingTop: 24 }}>
                    <div className={styles.card} style={{ padding: '32px' }}>
                        <h3 className={styles.sectionHeading} style={{ paddingTop: 0, marginBottom: '16px' }}>Technical Skills</h3>
                        <div className={styles.skillTags}>
                            {userSkills.length > 0 ? userSkills.map(s => <span key={s} className={styles.skill}>{s}</span>) : <p style={{ color: '#98A2B3' }}>No skills added yet.</p>}
                        </div>
                    </div>
                </div>
            )}
            ...
            {activeTab === 'education' && (
                <div style={{ paddingTop: 24 }}>
                    {education.map((edu, i) => (
                        <div key={i} className={styles.card} style={{ display: 'flex', gap: 14, alignItems: 'flex-start', marginBottom: '16px', padding: '24px' }}>
                            ...
                            <div>
                                <h3 style={{ fontSize: '1.125rem', fontWeight: 600, color: '#101828', marginBottom: 4 }}>{edu.degree}</h3>
                                <p style={{ fontSize: '0.875rem', color: '#475467', margin: 0 }}>{edu.institution}</p>
